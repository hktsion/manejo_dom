...
//Constantes que uso a lo largo del script
...
//mi section dentro del DOM donde luego voy a meter otros nodos;
let sectiondom;


//OBJETO DON :: Soporta todos los Nodos;
let dom = {
	acumDOM: CONSTDOM.init_acum,

	//carga del script cuando el DOM está cargado;
	load: function () {
		document.addEventListener('DOMContentLoaded', ( ) => { 
			
			this.addToDOM();		//Añade la tabla y el h2;
			this.addProperties();	//Añade un width que no estaba añadido en el CSS original;
			this.addListerners();	//Añade los listeners a los botones;
	...
	insertButton: function(){
		...
		//cuando se lance, genera botones y agrega el día mediante el array de datos;
		for (var i = 0; i < datos.length; i++) {
			let btn = table.appendDOM(this.acumDOM, 'button');
			btn.innerHTML = (datos[i][0].id).substr(0,2);
			btns.push(btn);
		};
		return btns; //devuelve los botones para usarlos más adelante y asociiarlos a un listener;
	},

	addListerners: function(){
		let btns = this.insertButton(); 	//Aquí se lanza => inserta los botones (no antes, los botones se generan en esta función );
		let tren = null;
				...
				for (let i = 0; i < datos.length; i++) {
					for (let j = 0; j < datos[i].length; j++) {
						...
						if((datos[i][j].id).substr(0,2) == this.innerText){
							tren = datos[i]; 	//Si los 2 dígitos del tren conciden con el texto que tene el botón => ya tengo el tren;
						}
					};
				};
				table.drop(); 	//elimina la tabla incial (si no exste, no pasa nada, no da error);
				table.insert(tren);	//regenera la tabla con el nuevo contenido;
			}, true);
		});
	},
}
...
let table = {
	...
	insert: function (tren){
		if(tren instanceof Object && tren != null){
			...
			//Añade el body de la tabla con los tr y td;
